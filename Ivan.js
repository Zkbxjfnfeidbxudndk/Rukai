...
const {DisconnectReason, useMultiFileAuthState, MessageRetryMap, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, jidNormalizedUser} = await import('baileys-pro');
...
// استخدام JSONFile فقط لتجنب مشكلة cloudDBAdapter غير المعرّف
...
global.reload = async (_ev, filename) => {
    if (pluginFilter(filename)) {
        ...
        if (filename in global.plugins) {
            if (fs.existsSync(dir)) conn.logger.info(` updated plugin - '${filename}'`);
            else {
...
function purgeSessionSB() {
    try {
        const listaDirectorios = fs.readdirSync(`./${jadi}/`);
        let SBprekey = [];
        listaDirectorios.forEach(directorio => {
            if (fs.statSync(`./${jadi}/${directorio}`).isDirectory()) {
                const DSBPreKeys = fs.readdirSync(`./${jadi}/${directorio}`).filter(fileInDir => {
                    return fileInDir.startsWith('pre-key-');
                });
                SBprekey = [...SBprekey, ...DSBPreKeys];
                DSBPreKeys.forEach(fileInDir => {
                    if (fileInDir !== 'creds.json') {
                        fs.unlinkSync(`./${jadi}/${directorio}/${fileInDir}`);
                    }
                });
            }
        });
...
function purgeOldFiles() {
    const directories = [`./${global.sessions}/`, `./${jadi}/`];
