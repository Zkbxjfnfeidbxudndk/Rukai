...
if (m.moras)
conn.reply(m.chat, `ستعملت *${+m.moras}* 🪙`, m, fake)
}
if (m.money)
conn.reply(m.chat, `ستعلمت *${+m.money}* 💰`, m, fake)
break
}}} catch (e) {
console.error(e)
} finally {
...
if (db.data.chats[m.chat].reaction && m.text.match(/(ميليوداس|ايرين|بوت|اوامر|لوفي|روكيا|menu|tion|age|ous|ate|and|bot|ify|goku|black|GokuBlack|a|s)/gi)) {
...
const msg = {
  rowner: 
`𝅄︶︶  ͝ ͝ ⏝ ⊹ ⏝ ︶ ͝   ͝ ︶𝅄 ㅤـ\n` +
`╰⊰⊹☀️⊹⊱╮  ⟨هذه الميزة مخصصة فقط لمنشئ البوت!☇⟩\n` +
`𝅄͝ ͝ ⏝ ⊹ ⏝︶ـ\n` +
`𝅄︶︶  ͝ ͝ ⏝ ⊹ ⏝ ︶ ͝   ͝ ︶𝅄 ㅤـ`,

  owner: 
`⃔⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌\n` +
`╰⊰🎩⊱╮ ۬˖ ⟨هذه الميزة مخصصة فقط لصاحب البوت!☇⟩\n` +
`𝅄︶⏝ ⊹ ⏝ ︶ ͝   \n` +
`⃔⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌ـ`,

  mods: 
`╮─ׅ─๋︩︪─┈─๋︩︪─═⊐‹👥›⊏═┈ ─๋︩︪─ ∙ ∙ ⊰ـ\n` +
`│┊ ۬.͜ـ🤴🏻˖ ⟨هذه الميزة مخصصة للمطورين فقط!☇⟩\n` +
`┤└─ׅ─ׅ┈ ─๋︩︪──ׅ─ׅ┈ ─๋︩︪☇ـ\n` +
`╯─ׅ─═⊐‹📛›⊏═┈ ─๋︩︪─ ∙ ∙ ⊰ـ`,

  premium: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ🍧˖ ⟨هذه الميزة للمستخدمين المميزين فقط!☇⟩\n` +
`︵‿︵‿୨♡୧‿︵‿︵ـ\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────ـ`,

  group: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────ـ\n` +
`╰⊰⊹🍨⊹⊱╮ ۬˖ ⟨هذه الميزة تعمل داخل المجموعات فقط!☇⟩\n` +
`𝅄͝ ͝ ⏝ ⊹ ⏝ ︶ ͝   ͝ـ\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────ـ`,

  private: 
`𝅄︶︶  ͝ ͝ ⏝ ⊹ ⏝ ︶ ͝   ͝ ︶𝅄\n` +
`╰⊰⊹✨️⊹⊱╮˖ ⟨هذه الميزة تعمل في الخاص فقط!☇⟩\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`𝅄︶︶  ͝ ͝ ⏝ ⊹ ⏝ ︶ ͝   ͝ ︶𝅄 ㅤـ`,

  admin: 
`⃔⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌ـ\n` +
`╰⊰⊹👑⊹⊱╮ ۬˖ ⟨هذه الميزة خاصة بمشرفي المجموعة فقط!☇⟩\n` +
`𝅄 ͝ ⏝ ⊹ ⏝ ︶ ͝  ـ\n` +
`⃔⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔‌⏝‌⃔⃔‌⏝‌⃔⃔‌ـ`,

  botAdmin: 
`₊˚‧ ︵‿₊୨🦋୧₊‿︵ ‧˚₊ ⊹ ︶⏝  ـ\n` +
`╰⊰⊹‼️⊹⊱╮ ۬.͜ـ˖ ⟨يجب أن أكون مشرفًا لاستخدام هذه الميزة!☇⟩\n` +
`︵‿︵‿୨♡୧‿︵‿︵\n` +
`𝅄︶︶  ͝ ͝ ⏝ ⊹ ⏝ ︶ ͝   ͝ ︶𝅄 ㅤـ`,

  unreg: 
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`୨🎶୧ ˖ ⟨مرحبًا! أنت غير مسجل، استخدم الأمر التالي للتسجيل:☇⟩\n` +
`୨🧸୧ ⟨*.تسجيل اسمك.عمرك*⟩\n` +
`୨🪽୧ ⟨مثال:*.تسجيل alastor.16*⟩\n` +
`─── ꒰ა⋟﹏⋞໒꒱ ───ـ\n` +
`𝅄︶︶  ͝ ͝ ⏝ ⊹ ⏝ ︶ ͝   ͝ ︶𝅄 ㅤـ`,

  restrict: 
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵ـ\n` +
`╰⊰⊹🍡⊹⊱╮˖ ⟨هذه الميزة مقيدة حاليًا من قبل المطور!☇⟩\n` +
`┤𝅄︶͝ ⏝ ⊹ ⏝︶ ͝   𝅄☇ـ\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵`
}[type];
