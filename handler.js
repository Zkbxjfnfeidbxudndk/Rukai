...
${user.bannedReason}` : '💌 *سبب:*غير محدد'}\n\n⚠️ *إذا كان هذا الروبوت حسابًا رسميًا ولديك دليل يدعم أن هذه الرسالة خطأ، فيمكنك عرض قضيتك على :*\n\n🤍 ${asistencia}`)
user.antispam++        
return
}
...
const msg = {
  rowner: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ👑˖ ⟨هذه الميزة مخصصة فقط لمنشئ البوت!☇⟩\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,

  owner: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ🛠️˖ ⟨هذه الميزة مخصصة فقط لصاحب البوت!☇⟩\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,
...
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,
  ...
  group: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ👬˖ ⟨هذه الميزة تعمل داخل المجموعات فقط!☇⟩\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,

  private: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ📥˖ ⟨هذه الميزة تعمل في الخاص فقط!☇⟩\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,

  admin: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ👑˖ ⟨هذه الميزة خاصة بمشرفي المجموعة فقط!☇⟩\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,

  botAdmin: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ🚩˖ ⟨يجب أن أكون مشرفًا لاستخدام هذه الميزة!☇⟩\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,

  unreg: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ📝˖ ⟨مرحبًا! أنت غير مسجل، استخدم الأمر التالي للتسجيل:☇⟩\n` +
`│┊ ⟨*.تسجيل اسمك.عمرك*⟩\n` +
`│┊ ⟨مثال:*.تسجيل alastor.16*⟩\n` +
`︵‿︵‿︵‿୨♡୧‿︵‿︵‿︵\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`,

  restrict: 
`───── ꒰ა⋟﹏⋞໒꒱ ─────\n` +
`│┊ ۬.͜ـ💫˖ ⟨هذه الميزة مقيدة حاليًا من قبل المطور!☇⟩\n` +
`𝅄︶︶  ͝ ͝ ⏝ ⊹ ⏝\n` +
`───── ꒰ა⋟﹏⋞໒꒱ ─────`
}[type];
